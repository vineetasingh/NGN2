...
void filterHessian(string imname, Mat image, ofstream &myfile)
{
	...
	string name = format("C:\\CCHMC\\NGN2\\STEPi\\Segmented_Orignal\\%s_dend_processed.png", imname.c_str());
	imwrite(name, org);
...
void findsoma(Mat im, Mat imr, vector<vector<Point>> blue_contours, vector<vector<Point>> intersection_contours, ofstream &myfile, string fstr)
{
	...
	else
	{
		...
		string out_string = format("C:\\CCHMC\\NGN2\\STEPi\\Segmented_Orignal\\%s_blue_processed.png", fstr.c_str());
...
int main(int argc, char** argv)
{
	...
	while (std::getline(file, fstr))
	{
		...
		for (int n = 1; n <= 40; n++)//differnt z scales
		{
			...
			if (n < 10)
				name = format("C:\\CCHMC\\NGN2\\STEPi\\%s\\%s_z0%dc1+2+3.jpg", fstr.c_str(), fstr.c_str(), n);
			else
				name = format("C:\\CCHMC\\NGN2\\STEPi\\%s\\%s_z%dc1+2+3.jpg", fstr.c_str(), fstr.c_str(), n);
			...
			string result = format("C:\\CCHMC\\NGN2\\STEPi\\Segmented_Orignal\\%s_processed.png", fstr.c_str(), fstr.c_str());
	...
	POP:
		...
		Mat temp;
		resultb = changeimg(resultb, 10, 5);
		resultr = changeimg(resultr, 50, 5);//final MIP, enhace images (resultr=red, resultb=blue, resultg=green)
		resultg = changeimg(resultg, 150, 0);
		temp = resultg;
		resultg = resultr;
		resultr = temp;
		//	imshow(" mip",result);
		string stringb = format("C:\\CCHMC\\NGN2\\STEPi\\Segmented_Orignal\\%s_blue_original.png", fstr.c_str());
		string stringg = format("C:\\CCHMC\\NGN2\\STEPi\\Segmented_Orignal\\%s_green_original.png", fstr.c_str());
		string stringr = format("C:\\CCHMC\\NGN2\\STEPi\\Segmented_Orignal\\%s_red_original.png", fstr.c_str());
		...
		//detecting dendrites
		/*~~~~~~~*/filterHessian(imagnam, resultg, myfile);
		...
		Mat C, ttt;
		addWeighted(resultb, 0.8, resultg, 0.2, 5, C);

		//thresholding nuclei (blue) in the image
		cv::Mat element = cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(5, 5));
		cv::inRange(resultb, cv::Scalar(80, 0, 0), cv::Scalar(150, 10, 10), bthr);//BLUE THRESH
